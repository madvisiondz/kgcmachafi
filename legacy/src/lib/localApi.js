...
export const adminApi = {
  ...
  getI18n: async ({ entity_type, entity_id, lang }) => {
    const params = new URLSearchParams({
      entity_type: String(entity_type),
      entity_id: String(entity_id),
      lang: String(lang),
    });
    return request(`/api/admin/i18n.php?${params.toString()}`);
  },
  saveI18n: async ({ entity_type, entity_id, lang, fields }) =>
    request('/api/admin/i18n.php', {
      method: 'PUT',
      body: JSON.stringify({ entity_type, entity_id, lang, fields }),
    }),
};
...
export const contentApi = {
  ...
  getSiteContent: async ({ lang } = {}) => {
    try {
      const q = lang ? `?lang=${encodeURIComponent(lang)}` : '';
      return await request(`/api/public/site-content.php${q}`);
    } catch (error) {
