...
- Every page built in the new frontend should get its own dedicated `.md` tracker file under **`../TRACKERS/machafi-services/`** (Machafi Services), **`../TRACKERS/machafi-tv/`** (Machafi TV), **`../TRACKERS/machafi-services-admin/`** (Health Services admin), or **`../TRACKERS/machafi-tv-admin/`** (Machafi TV admin), matching the product surface.
