...
- **Prompt**: Create trackers for admin panels.
- **What I did**: Added **`TRACKERS/machafi-services-admin/HEALTHSERVICES_ADMIN_PANEL_MAP.md`** (`/healthservices/admin/*`) and **`TRACKERS/machafi-tv-admin/MACHAFITV_ADMIN_PANEL_MAP.md`** (`/machafitv/admin/*`); updated **`WEBAPP_PAGES_OVERVIEW.md`**, **`README.md`**, **`PROJECT_STATUS.md`**; appended **`PROMPT_LOG.md`**.

### Project memory — `PROJECT-EXPLAINER/` folder
- **Prompt**: New folder **`PROJECT-EXPLAINER`**; move the rest of root **`.md`** files into it (keep **`README.md`** at repo root).
- **What I did**: Created **`PROJECT-EXPLAINER/`** and moved all root markdown except **`README.md`** (including **`project-explainer.md`**, **`PROJECT_STATUS.md`**, **`PROMPT_LOG.md`**, maps, rules, diagrams). Updated **`TRACKERS/**`** footers to `../../PROJECT-EXPLAINER/PAGE_DATASET_REFERENCE.md` and `../../PROJECT-EXPLAINER/PROMPT_LOG.md`; updated Rule #0 cross-links in trackers to **`../../PROJECT-EXPLAINER/…`**; normalized **`PROJECT-EXPLAINER/*.md`** references to **`../TRACKERS/…`** where appropriate; rewrote **`README.md`** memory list + repo layout; updated **`canvases/project-workflow.canvas.tsx`** table + tracking callout; appended **`PROMPT_LOG.md`**.
...
### Frontend routing — gateway, `/healthservices`, `/tv/:edition`, admin paths
- **Prompt**: Create the new paths and architecture.
- **What I did**: Added **`frontend/src/routes/paths.ts`** (`SERVICES_BASE`, `servicesPath`, `tvEditionPath`); **`GatewayPage`** at **`/`** with optional remember-shell **`localStorage`**; nested **`ServicesLayout`** + all former routes under **`/healthservices/*`**; **`TvShellLayout`** at **`/tv/:edition`** with **`TvHomePage`**, **`TvLivePage`**, **`TvSchedulePage`**, **`TvArticlePage`** (stubs); **`HealthServicesAdminPage`** / **`MachafiTvAdminPage`** at **`/healthservices/admin/*`** and **`/machafitv/admin/*`**; legacy **`Navigate`** redirects from old flat paths; updated **Header**, **Footer**, **DocumentTitle**, and page **`Link`** targets via **`servicesPath()`**; i18n **`gateway.*`**, **`tvApp.*`**, **`admin.*`** (AR/FR/EN); **`npm run build`** OK; updated **`PLATFORM_SHELL_LAYOUT.md`** + **`WEBAPP_PAGES_OVERVIEW.md`** + **`PROJECT_STATUS.md`**.

### Routing switch — briefing doc
- **Prompt**: Create a `.md` briefing of what had to be done for the switch (last routing prompt).
- **What I did**: Added **`ROUTING_SWITCH_BRIEFING.md`** (goals, URL map, files touched, deploy checklist, out-of-scope next steps); linked from **`README.md`** project memory list.

### Machafi TV — desk, wire activity, topics, search + live/schedule/article
- **Prompt**: Finish the TV frontend for `/tv/ar|fr|en`: tools/services, newswire-style activity, industry-style pages.
- **What I did**: Added **`TvDeskPage`**, **`TvActivityPage`** (`tvWireActivity`), **`TvTopicPage`** (`topics/:topicId`), **`TvSearchPage`** (`?q=`); wired routes in **`App.tsx`**; extended **`DocumentTitle`** for desk/activity/search/topics; implemented **`TvLivePage`**, **`TvSchedulePage`**, **`TvArticlePage`** (body paragraphs, share/print/correction row); fixed **`TvBreakingBar.jsx`** (removed invalid `import type` / TS props in JSX); **`npm run build`** OK.

### Page trackers — `TRACKERS/` folder (Machafi Services vs Machafi TV)
- **Prompt**: Create **`TRACKERS/`** with two branch directories; move all page tracker `.md` files into the correct branch folder.
- **What I did**: Added **`../TRACKERS/machafi-services/`** (13 files: `HOMEPAGE_MAP.md`, `*_PAGE_MAP.md` for Services routes) and **`../TRACKERS/machafi-tv/`** (9 `TV_*_PAGE_MAP.md` files); updated cross-links inside trackers (relative paths to `../../PROMPT_LOG.md`, `../../PAGE_DATASET_REFERENCE.md`, and sibling-branch `../machafi-services/…` where needed); updated **`README.md`**, **`WEBAPP_PAGES_OVERVIEW.md`**, **`PROJECT_STATUS.md`**, **`LEGACY_SYSTEM_PROBLEMS_AND_REBUILD_RULES.md`**, **`project-explainer.md`**, **`HOW_DOES_A_PRO_AI_WEB_DEVELOPER_DO_THAT.md`**, **`HEADER_SCROLL_ANIMATION.md`**; appended **`HOMEPAGE_MAP.md`** file-location line.

### Machafi TV — page tracker markdown files
- **Prompt**: Create a tracker `.md` for every TV page created, like existing repo `*_PAGE_MAP.md` files.
- **What I did**: Added under **`../TRACKERS/machafi-tv/`**: **`TV_SHELL_PAGE_MAP.md`** (shell + breaking bar), **`TV_HOME_PAGE_MAP.md`**, **`TV_LIVE_PAGE_MAP.md`**, **`TV_SCHEDULE_PAGE_MAP.md`**, **`TV_ARTICLE_PAGE_MAP.md`**, **`TV_DESK_PAGE_MAP.md`**, **`TV_ACTIVITY_PAGE_MAP.md`**, **`TV_TOPIC_PAGE_MAP.md`**, **`TV_SEARCH_PAGE_MAP.md`**; updated **`WEBAPP_PAGES_OVERVIEW.md`** (TV route table + readiness) and **`README.md`** (memory list); appended **`PROMPT_LOG.md`**.
