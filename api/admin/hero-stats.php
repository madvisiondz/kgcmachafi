...
if ($method === 'POST') {
    ...
    $fetch->execute(['id' => $id]);
    json_response(['message' => 'تمت إضافة الإحصائية.', 'item' => $fetch->fetch()], 201);
}
...
if ($method === 'PUT') {
    ...
    $fetch = db()->prepare('SELECT * FROM hero_stats WHERE id = :id');
    $fetch->execute(['id' => $id]);
    json_response(['message' => 'تم تحديث الإحصائية.', 'item' => $fetch->fetch()]);
}
...
if ($method === 'DELETE') {
    ...
    json_response(['message' => 'تم حذف الإحصائية.']);
}
